...
def all_products(db:Session=Depends(get_db)):
    productos = db.query(ProductInDB).filter(ProductInDB.estado == True).all()
    return productos
...
def delete_products(id_producto:str,db:Session = Depends(get_db)):
    producto = db.query(ProductInDB).filter(ProductInDB.id_producto == id_producto, ProductInDB.estado == True).first()
    compra = db.query(CompraInDB).filter(CompraInDB.id_producto == id_producto, CompraInDB.estado == True).first()
